...
cmd({
  ...
  try {
    ...
    if (containsLink && config.ANTI_LINK === 'true') {
      console.log(`Link detecter ${sender}: ${body}`);
...
text: `‎*⚠️ LES LIENS SONT INTERDITS ⚠️*\n` +
      ...
      `*╰──────────────────────────*`,
        ...
        await conn.sendMessage(from, {
          text: `@${sender.split('@')[0]} *A ÉTÉ SUPPRIMÉ - ATTENTION LIMITE DÉPASSÉE !*`,
          mentions: [sender]
        });
    ...
    console.error("Anti-lien:", error);
    reply("❌Une erreur est survenue l'ors du processus.");
  }
});
