...
`;
        ...
        const imageUrl = "https://files.catbox.moe/5o09h5.png"; // Remplace "TON_LIEN_IMAGE_ICI" par ton lien d'image
        ...
        await conn.sendMessage(m.chat, {
            image: { url: imageUrl },
            caption: `📖 *BIBLE LIST BY 𝔻𝔸ℝ𝕂-𝔹𝕆𝕐*:\n\n` +
                     `Here is the complete list of books in the Bible:\n\n` +
                     bibleList.trim() // Ajout du texte des livres de la Bible
        }, { quoted: mek });
    } catch (error) {
        console.error(error);
        reply("❌ *Une erreur s'est produite lors de la récupération de la liste des Bibles. Veuillez Essayer.*");
    }
});
