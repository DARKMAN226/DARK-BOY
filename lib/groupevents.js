...
const GroupEvents = async (conn, update) => {
    try {
        ...
        for (const num of participants) {
            ...
            if (update.action === "add" && config.WELCOME === "true") {
                const WelcomeText = `Salut @${userName} 👋\n` +
                    ...
                    `Merci de lire la description du groupe pour éviter d'être retiré :\n` +
                    `${desc}\n` +
                ...
                await conn.sendMessage(update.id, {
                    text: `*Événement Admin*\n\n` +
                          `@${demoter} a retiré les droits d'admin à @${userName} 👀\n` +
                          `Heure : ${timestamp}\n` +
        ...
        console.error('Erreur lors d’un événement de groupe :', err);
    }
};
